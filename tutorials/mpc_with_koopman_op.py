...
X_ic = rng.uniform(-3., 3., size=(n_timeseries, 2))
...
U_tsc = rng.uniform(-3., 3., size=(n_timeseries, 1, 1))
U_tsc = np.tile(U_tsc, (1, n_timesteps - 1, 1))
U_tsc = TSCDataFrame.from_tensor(U_tsc, time_series_ids=X_ic.ids, columns=vdp.control_names_in_, time_values=time_values[:-1])
...
edmd = EDMD(dict_steps=[("vdpdict", VdPDictionary())], dmd_model=DMDControl(), include_id_state=True)
...
target_state = InitialCondition.from_array(np.array([0, 0]), feature_names=edmd.feature_names_in_, time_value=0)
...
X_oos = TSCDataFrame.from_array(np.zeros((n_timesteps_oos, 2)), feature_names=vdp.feature_names_in_, time_values=time_values_oos)
U_oos = TSCDataFrame.from_array(np.zeros((n_timesteps_oos - 1, 1)), feature_names=vdp.control_names_in_, time_values=time_values_oos[:-1])
...
for i in tqdm(range(1, n_timesteps_oos)):
    ...
    new_state, _ = vdp.predict(state, U=U_oos.iloc[[i - 1], :], time_values=time_values_oos[i - 1:i + 1])
    X_oos.iloc[i, :] = new_state.iloc[[1], :].to_numpy()


trajectory_uncontrolled, _ = vdp.predict(X_ic_oos, U=np.zeros((n_timesteps_oos - 1)), time_values=time_values_oos)
...
plt.figure()
plt.plot(X_oos.loc[:, "x1"].to_numpy(), X_oos.loc[:, "x2"].to_numpy(), c="red", label="controlled system")
plt.quiver(*X_oos.to_numpy()[:-1, :].T, *np.column_stack([np.zeros_like(U_oos.to_numpy()), U_oos.to_numpy() / X_oos.delta_time]).T, color="blue")
plt.plot(X_oos.iloc[0, 0], X_oos.iloc[0, 1], "o", c="red")
plt.plot(trajectory_uncontrolled.loc[:, "x1"].to_numpy(), trajectory_uncontrolled.loc[:, "x2"].to_numpy(), c="black", label="uncontrolled system")
plt.plot(trajectory_uncontrolled.iloc[0, 0], trajectory_uncontrolled.iloc[0, 1], "o", c="black", label="initial state")
plt.plot(target_state.iloc[0, 0], target_state.iloc[0, 1], "*", c="black", label="target state state")
plt.xlabel('x1')
plt.ylabel('x2')
plt.legend()
