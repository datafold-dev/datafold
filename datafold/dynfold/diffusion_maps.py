...
class DiffusionMapsVariable(KernelMethod, TransformerMixin):
    ...
    def __init__(self,
        ...
        self.beta = beta
        self.k = k  # TODO: better name -- this is the number of neighbors to consider...

        # TODO: To implement? is_stochastic = True
        # TODO: To implement: cut_off: float = np.inf (allow also sparsity!)
        super(DiffusionMapsVariable, self).__init__(epsilon, num_eigenpairs, None, False, -1, symmetrize_kernel,
                                                    use_cuda, dist_backend, dist_backend_kwargs)

        self._kernel = DmapKernelVariable(epsilon=self.epsilon, k=k, expected_dim=expected_dim, beta=beta,
    ...
    def peq_est_(self):
        ...
        return self.q_eps_s_ / (num_samples * (4*np.pi*self.epsilon)**(self.expected_dim/2))
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        self._kernel_matrix, self._basis_change_matrix, self.rho0_, self.rho_, self.q0_, self.q_eps_s_ = \
            pcm.compute_kernel_matrix()
        self.eigenvalues_, self.eigenvectors_ = self.solve_eigenproblem(self._kernel_matrix, self._basis_change_matrix,
                                                                        self.use_cuda)
        self.eigenvalues_ = np.power(self.eigenvalues_, 1 / self.epsilon, out=self.eigenvalues_)
        self.eigenvectors_ = self.eigenvectors_ / np.linalg.norm(self.eigenvectors_, axis=1)[:, np.newaxis] * np.sqrt(pcm.shape[0])
...
class DiffusionMaps(KernelMethod, TransformerMixin):
    ...
    def __init__(self,
        ...
        super(DiffusionMaps, self).__init__(epsilon, num_eigenpairs, cut_off, is_stochastic, alpha,
                                            symmetrize_kernel, use_cuda, dist_backend, dist_backend_kwargs)

        self._kernel = DmapKernelFixed(epsilon=epsilon, is_stochastic=is_stochastic, alpha=alpha,
                                       symmetrize_kernel=symmetrize_kernel)
...
class LocalRegressionSelection(TransformerMixin):
    ...
    def __init__(self, eps_med_scale=3, n_subsample=np.inf, strategy="dim", intrinsic_dim=2, regress_threshold=0.9,
        ...
        if n_subsample < 100:
            raise ValueError(f"Parameter use_samples has to be larger than 100 samples. Got "
                             f"n_subsample={n_subsample}.")

        if strategy not in ["dim", "threshold"]:
            raise ValueError("")  # TODO

        if strategy == "dim" and (not isinstance(intrinsic_dim, numbers.Integral) or intrinsic_dim < 0):
        ...
        if strategy == "threshold" and \
            ...
            raise ValueError(f"'regress_threshold' has to be non-negative float value between (0, 1). "
                             f"Got {regress_threshold}")

        valid_bandwidth_types = ["median", "mean"]
        if bandwidth_type not in valid_bandwidth_types:
            raise ValueError(f"Valid options for 'locregress_bandwidth_type' are {valid_bandwidth_types}. "
    ...
    def _single_residual_local_regression(self, domain_eigenvectors, target_eigenvector):
        # Helper function for residuals_local_regression_eigenvectors

        nr_samples = domain_eigenvectors.shape[0]

        distance_matrix_eigvec = scipy.spatial.distance.pdist(domain_eigenvectors, 'euclidean')
        distance_matrix_eigvec = scipy.spatial.distance.squareform(distance_matrix_eigvec)
        ...
        if self.bandwidth_type == "median":
            eps_regression = np.median(np.square(distance_matrix_eigvec.flatten())) / self.eps_med_scale
        elif self.bandwidth_type == "mean":
            eps_regression = np.mean(np.square(distance_matrix_eigvec.flatten())) / self.eps_med_scale
        else:
        ...
        for i in range(nr_samples):  # TODO: this loop is extremly expensive...
            ...
            data_matrix = np.hstack([const_ones, domain_eigenvectors - domain_eigenvectors[i, :]])
            ...
            current_solution_coeff, res_, rank_, singular_values_ = \
                np.linalg.lstsq((weight_data_matrix @ data_matrix), weight_data_matrix, rcond=1E-6)
        ...
        residual = np.sqrt(np.sum(np.square((target_eigenvector - estimated_target_values))) /
    ...
    def fit(self, X, y=None):
        ...
        if num_eigenvectors <= 1:  # TODO: #44
            raise ValueError("There must be more than one eigenvector to compute the residual.")
        ...
        if self.strategy == "dim":
            if not (0 < self.intrinsic_dim < num_eigenvectors-1):
                ...
                raise ValueError(f"intrinsic_dim has to be an integer larger than 1 and smaller than num_eigenpairs-1="
                                 f"{num_eigenvectors}. Got intrinsic_dim={self.intrinsic_dim}")
        elif self.strategy == "threshold":
            if not (0 < self.regress_threshold < 1):
                raise ValueError(f"residual_threshold has to between [0, 1], exclusive. Got residual_threshold="
        ...
        self.residuals_[1] = 1       # the first eigenvector is always taken, therefore receives a 1

        for i in range(2, num_eigenvectors):
            self.residuals_[i] = self._single_residual_local_regression(domain_eigenvectors=eigvec[:, 1:i],
                                                                        target_eigenvector=eigvec[:, i])
    ...
    def transform(self, X, y=None):
        ...
        if self.strategy == "dim":
            if self.intrinsic_dim == 1:
                ...
                self.residuals_[0] = -1  # set the trivial cases invalid  # TODO: do this somewhere else?

                # NOTE: negative residuals to obtain the largest -- argpartition are the respective indices
                self.evec_indices_ = np.argpartition(-self.residuals_, self.intrinsic_dim)[:self.intrinsic_dim]
