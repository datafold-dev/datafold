...
class FeedforwardNN(DictLearningMethod):
    ...
    class _intern_FNN(torch.nn.Module):
        def __init__(
            ...
            for i in range(len(layer_sizes) - 1):
                layers.append(
                    torch.nn.Linear(
                        layer_sizes[i], layer_sizes[i + 1], bias=bias, dtype=use_dtype
                    )
                )
        ...
        def sample_init_weights(self, X):
            ...
            with torch.no_grad():
                for i in range(4):
                    self.model[i * 2].weight.copy_(torch.Tensor(pipeline[i].weights.T))
                    self.model[i * 2].bias.copy_(torch.Tensor(pipeline[i].biases[0].T))
    ...
    def loss_func(self, _lambda, psi_now, system_matrix, psi_next, inverse_map, X_now):
    ...
    def _validate_network(
        ...
        psi_now_aug, psi_next_aug = psi.tsc.shift_matrices(snapshot_orientation="row")
    ...
    def fit(self, X: TSCDataFrame, *, y=None, **fit_params):
        ...
        **fit_params
        ...
        optimizer = torch.optim.Adam(self.method.parameters(), lr=self.learning_rate)
        ...
        for epoch in all_epochs:
            ...
            trainloader = torch.utils.data.DataLoader(
                train_data_nn, batch_size=self.batch_size, shuffle=True, num_workers=1
            )

            # train network
            current_train_loss = self._train_epoch(
                system_matrix, inverse_map, trainloader=trainloader, optimizer=optimizer
            )
