...
__all__ = [
    "plot_diffusion_maps", "plot_spectrum", "plot_results", "plot_l_vs_epsilon", "plot_l_vs_epsilon_heuristic",
    "plot_eps_vs_error", "plot_number_eigenvectors_vs_error"
]
...
def plot_data_colored_by_diffusion_maps(data: np.ndarray, eigenvectors: np.ndarray) -> None:
    """Plot colored data.
...
def plot_number_eigenvectors_vs_error(points: np.ndarray, values: np.ndarray,
                                      epsilon: float, k: int = 4,
                                      number_of_eigenpairs: np.ndarray = np.arange(2, 11)) -> None:
...
def plot_eps_vs_error(points: np.array, values: np.array, epsilons: np.ndarray, num_eigenpairs: int, k: int = 4) -> None:
...
def _plot_eigenvectors(eigenvector1: np.ndarray, eigenvector2: np.ndarray, colors: np.ndarray, ax: plt.Axes,
                       title: str) -> None:
...
def plot_eigenvectors_n_vs_all(eigenvectors: np.ndarray, n: int, colors: Optional[np.ndarray] = None) -> None:
    ...
    for i in range(2, int(eigenvectors.shape[0])):
        ...
        _plot_eigenvectors(eigenvectors[n, :],
                           eigenvectors[i, :],
                           colors,
...
def plot_results(data: np.ndarray, eigenvalues: np.ndarray, eigenvectors: np.ndarray, colors: np.ndarray,
                 spectrum: bool = True, colored_data: bool = True, diffusion_maps_2d: bool = True) -> None:
...
def plot_diffusion_maps(data: np.ndarray, dmaps: DiffusionMaps, colors: np.ndarray = 'b') -> None:
    """Plot diffusion maps.
...
def plot_l_vs_epsilon(data: np.ndarray, epsilons_coarse: List[float] = None, epsilons_fine: List[float] = None,
                      epsilon_magnitude: int = None, epsilon: float = None,
                      lut_km_coarse: "LookupKernelMatsEpsilon" = None,
                      lut_km_fine: "LookupKernelMatsEpsilon" = None) -> None:
    """Plot L(epsilon).
    ...
    if epsilons_coarse:
        if lut_km_coarse:
            ...
            if epsilon_magnitude is not None:
                index_epsilon = epsilons_coarse.index(round(10 ** epsilon_magnitude, -epsilon_magnitude))
...
def log_midpoint_rule(data: np.ndarray) -> float:
    ...
    log_midpoint = (np.log(dat_min) + np.log(data_range)) / 2.
    return float(log_midpoint)


def plot_l_vs_epsilon_heuristic(data: np.ndarray, epsilon_min_magnitude: int, epsilon_max_magnitude: int) -> None:
    """
    ...
    if epsilon_max_magnitude <= epsilon_min_magnitude:
        raise ValueError('epsilon_max_magnitude must be larger than epsilon_min_magnitude')

    epsilons_fine = (10 ** np.arange(epsilon_min_magnitude, epsilon_max_magnitude, 0.25)).tolist()
    lut_km_fine = LookupKernelMatsEpsilon(data, epsilons_fine)
    ...
    epsilon = epsilon_1
    plot_l_vs_epsilon(data, epsilons_fine=epsilons_fine, lut_km_fine=lut_km_fine, epsilon=epsilon)
...
def _ls_of_kernel_matrices(kernel_matrices: "LookupKernelMatsEpsilon") -> np.ndarray:
    ...
    return np.array([kernel_matrices(eps) for eps in kernel_matrices.epsilons]).sum(axis=(1, 2))
...
class LookupDmapsEpsilon(object):
    ...
    def __init__(self, data: np.ndarray, epsilons: List[float], normalize_kernel: bool = True) -> None:
        ...
        self._epsilons = epsilons   # For each epsilon the corresponding diffusion maps object.

        self._dmaps: List[DiffusionMaps] = []
        for eps in epsilons:
            try:
                dmap = DiffusionMaps(epsilon=eps, num_eigenpairs=11, is_stochastic=normalize_kernel).fit(data)
...
class LookupKernelMatsEpsilon(object):
    ...
    def __init__(self, data: np.ndarray,
        ...
        self._kernel_mats = [KernelBase.apply_kernel(data, epsilon) for epsilon in self.epsilons]
