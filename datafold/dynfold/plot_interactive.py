...
def _checkboxes(descriptions: List[str], defaults: List[bool]) -> List[widgets.Checkbox]:
    ...
    for description, val in zip(descriptions, defaults):
        checkbox_list.append(widgets.Checkbox(value=val, description=description, disabled=False))
    return checkbox_list


def _plot_diffusion_maps(data: np.array, epsilon_magnitude: int, lut_dmaps: Callable[[float], DiffusionMaps] = None,
    ...
    epsilon = round(10. ** epsilon_magnitude, -epsilon_magnitude)
...
def plot_diffusion_maps_interactive_epsilon(data: np.array, epsilon_min_magnitude: int, epsilon_max_magnitude: int,
                                            colors: np.array = None) -> None:
    ...
    epsilons = [round(10. ** p, -p) for p in range(epsilon_min_magnitude, epsilon_max_magnitude + 1, 1)]
    lut_dmaps = LookupDmapsEpsilon(data, epsilons)
    epsilon_magnitude = widgets.IntSlider(min=epsilon_min_magnitude,
                                          max=epsilon_max_magnitude,
                                          step=1,
                                          continuous_update=False)

    out = widgets.interactive_output(_plot_diffusion_maps,
...
def plot_diffusion_maps_interactively(data: np.array, dmaps: DiffusionMaps, colors: np.array = None,
    ...
    spectrum, colored_data, diffusion_maps_2d = _checkboxes(['spectrum', 'data - eigenvector', 'diffusion_maps_2D'],
                                                            defaults)

    ui = widgets.HBox([spectrum, colored_data, diffusion_maps_2d])
    out = widgets.interactive_output(plot_results,
                                     {'data': widgets.fixed(data),
...
def plot_l_vs_epsilon_interactively(data: np.array, epsilon_min_magnitude: int, epsilon_max_magnitude: int) -> None:
    ...
    epsilons_coarse = [round(10. ** p, -p) for p in range(epsilon_min_magnitude - 1, epsilon_max_magnitude + 2, 1)]
    epsilons_fine = (10 ** np.arange(epsilon_min_magnitude - 1, epsilon_max_magnitude + 1.25, 0.25)).tolist()
    ...
    out = widgets.interactive_output(plot_l_vs_epsilon,
                                     {'data': widgets.fixed(data),
