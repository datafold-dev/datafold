...
class GeometricHarmonicsTest(unittest.TestCase):
    ...
    def test_geometric_harmonics_interpolator(self):
        ...
        eps = 1e-1
        dmaps_opts = {"num_eigenpairs": self.num_points - 3, "cut_off": 1e1 * eps}
        ghi = GeometricHarmonicsInterpolator(self.points, self.values, eps, dmaps_opts)
        ...
        with Clock() as clock:
            values = ghi(points)
        logging.debug(
            "Evaluation of geometric harmonics done ({} seconds).".format(clock)
        )
    ...
    def test_eigenfunctions(self):
        ...
        dm = DiffusionMaps(points, eps, cut_off=cut_off, num_eigenpairs=num_eigenpairs)
        ev = dm.eigenvectors
