...
class BaseDiffusionMaps:
    ...
    def solve_eigenproblem(
        kernel_matrix, num_eigenpairs: int, use_cuda: bool
...
class SparseDiffusionMaps(BaseDiffusionMaps):
    ...
    def compute_distance_matrix(self) -> scipy.sparse.coo_matrix:
        ...
        """
        distance_matrix = self._kdtree.sparse_distance_matrix(
            self._kdtree, self._cut_off, output_type="coo_matrix"
        )
    ...
    def compute_kernel_matrix(
        self, distance_matrix: scipy.sparse.spmatrix
...
class DenseDiffusionMaps(BaseDiffusionMaps):
    def __init__(
        ...
        distance_matrix_squared = scipy.spatial.distance.pdist(
            points, metric="sqeuclidean"
        )  # noqa
        distance_matrix_squared = scipy.spatial.distance.squareform(
        ...
        if use_cuda is True:
            ...
            warnings.warn(
                "Dense diffusion maps are not implemented on the "
                "GPU. Using the CPU instead."
            )

        ew, ev = self.solve_eigenproblem(
            self.kernel_matrix, num_eigenpairs, use_cuda=False
        )
        if np.linalg.norm(ew.imag > 1e2 * sys.float_info.epsilon, np.inf):
            raise ValueError("Eigenvalues have non-negligible imaginary part")
        self.eigenvalues = ew.real
    ...
    def normalize_kernel_matrix(
        self, matrix: np.array, alpha: Optional[float] = 1
    ) -> np.array:
        assert 0 <= alpha <= 1, "Invalid normalization exponent."
