...
def main():
    parser = argparse.ArgumentParser(description="computation of diffusion " "maps")
    parser.add_argument(
        "data_file",
        metavar="FILE",
        type=str,
        help="process %(metavar)s (admits NumPy, MATLAB, and " "CSV formats)",
    )
    parser.add_argument("epsilon", metavar="VALUE", type=float, help="kernel bandwidth")
    parser.add_argument(
        "-b",
        "--bounds",
        type=str,
        required=False,
        metavar="FILE",
        help="Vector of upper bounds for " "periodic boxes",
    )
    parser.add_argument(
        "-n",
        "--num-samples",
    ...
    parser.add_argument(
        ...
        help="number of eigenvalue/eigenvector pairs to " "compute",
    )
    parser.add_argument(
        "-c",
        "--cut-off",
        ...
        required=False,
        metavar="DISTANCE",
        help="cut-off to use in order to " "enforce sparsity",
    )
    parser.add_argument(
        "-r",
        "--renormalization",
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Use dense instead of sparse linear algebra " "routines",
    )
    parser.add_argument(
        "-o",
        "--output-data",
        ...
        metavar="FILE",
        help="save " "actual data used in computation to %(metavar)s",
    )
    parser.add_argument(
        "-w",
        "--eigenvalues",
        ...
        required=False,
        metavar="FILE",
        help="save eigenvalues to " "%(metavar)s",
    )
    parser.add_argument(
        "-v",
        "--eigenvectors",
        ...
        required=False,
        metavar="FILE",
        help="save eigenvectors to " "%(metavar)s",
    )
    parser.add_argument(
        "-m",
        "--matrix",
        ...
        required=False,
        metavar="FILE",
        help="save transition matrix to " "%(metavar)s",
    )
    parser.add_argument(
        "-p",
        "--plot",
    ...
    parser.add_argument(
        ...
        help="run under profiler and save report to " "%(metavar)s",
    )
    ...
    if args.num_samples is not None:
        ...
        if num_samples <= orig_data.shape[0]:
            ...
            logging.warning(
                "Data set contains {} points but {} points "
                "were requested".format(orig_data.shape[0], num_samples)
            )
            sys.exit(-1)
    else:
        data = orig_data
    ...
    logging.info(
        "Computing {} diffusion maps with epsilon = {:g} "
        "on {} data points...".format(
            args.num_eigenpairs - 1, args.epsilon, data.shape[0]
        )
    )
    ...
    if np.linalg.norm(dm.eigenvectors.imag, np.inf) > threshold:
        logging.warning(
            "Eigenvectors have a non-negligible imaginary part. "
    ...
    if args.output_data and args.num_samples:
        logging.info("Saving downsampled data to {!r}".format(args.output_data))
