...
class PCMEstimatonUnitTests(unittest.TestCase):
    ...
    def test_llr1(self):
        ...
        dependent_vectors = np.column_stack([u[:,0], u[:, 0]*2, u[:, 0]**2+u[:, 0]] )
        independent_vectors = np.column_stack([u[:, 1], u[:, 2]*2, u[:, 2]**2+u[:, 3]] )
        
        all_vectors = np.column_stack([dependent_vectors, independent_vectors])
        
        pcme = PCMEstimation
        eps = 1
        result = pcme.compute_residuals(all_vectors, eps_scale=eps, progressBar=False,
                                                   skipFirst=False, bandwidth_type='median')
        
        allutils._assert_eq_matrices_tol(np.array([1,0,0,1,1,1]), result['Residuals'], tol=1E-1)
