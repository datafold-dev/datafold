...
Increasing data availability has changed the analysis and interpretation of data in many scientific fields and applications. While analyzed complex systems, such as a pyhsical systems, remain the same, the number of data measurements increase both in quantity and dimension. The increased data availability can extent the analysis and in a best case provide new insights for systems with unknown or untractable principal equations. A path can be the direct analysis of underyling intrinsic geometrical structure that a model would aim to extent for unseen data. A data-driven model may as well serve as a surrogate for complex application tasks, such as future time predictions and regression.

In contrast to the equation-driven analysis, a data-driven analysis can decrease the research time and cost of finding the principal equations. However, the increasing data availability also creates challanges in the entire analysis workflow from processing, extracting, to interpreting the data. 

In mathematical terms the geometric structure encoded in the data is referred to as a "manifold". All data-driven models assume that there is some pattern or manifold structure in the data. Machine learning is then the process of building numerical models which parameters adapt to this underyling structure and solve tasks like regression, classification or prediction. We can classify machine learning models in how they incorporate and presume the geometrical form. On one hand black box models have an implicit presumption of structure in the data, but do not allow this insight to be revealed. A canonical example of black box model is the feedforward neural networks, which can provide great predictive power as it adapts to to highly non-linear data structures. But due to it's construction it does not reveal the intrinsic manifold information. On the other hand, there are model types that explicitly incorporate manifold structure in the data. A parametrization of a data manifold which also generalizes to regions outside the available data samples, can then provide a path to uncover information about the intrinsic behaviour of the underlying system. An advantage of an explicit incorporation of the data structure is that it becomes possible to include prior knowledge of the data and system being analyized. For example, a selected kernel can describes the proximity between samples and therefore includes information about the hidden manifold.

datafold is a Python package providing **data**-driven models with an explicit mani**fold** parametrization. It provides some popular models, but with the software architecture and a clear modularization (taken by example from scikit-learn). We want to address the ongoing and active research in this field and welcome researchers in the scope of datafold to incorporate new ideas and model adaptation. The architecture design of datafold addresses the circumstance that models can range from higher level tasks (e.g., prediction |FIND| or building surrogate models |FD|) to lower level tasks, such as defining a kernel to encode locality information on a manifold |Giannakis_cone_kernel|. Furthermore, the software targets experienced practitioners and researchers from different fields to analyze their datasets. 

There are two main data type cases addressed in datafold that we describe in the following sections. Both types reflect also branches of strong research of fields with a manifold assumption.
...
The first type of data are sampled high-dimensional point clouds. The data-driven models then extract the non-linear manifold structure from high-dimensional point cloud data. In machine learning context this is also referred to "non-linear unsupervised learning" or shorter "manifold learning". Manifold learning is directly connected to the "manifold assumption". It states that a high-dimensional point cloud is assumed to lie on an intrinsic lower dimensional manifold. Extracting the latent and parsinomenous representation of the data reduces the burden of the "curse of dimensionality" and can increase the accuracy of classification or regression models. Often methods of this class are endowed with a kernel, such as the  "kernel principal comonent analysis" (Kernel PCA) |cite|, "Diffusion Maps" |cite|, or "Locally Linear Embedding" (LLE) |cite|. A number of manifold learning algorithms is also provided by scikit-learn. 

An important further issue in many applications that apply manifold learning models is to extent the image and/or pre-image mapping between the original and latent space to unseen data. This mapping is in genreal non-linear and therefore requires a separate model that performs this mapping (see for a analysis of different models |cite Cassimonous, Yannis|). These so-called out-of sample methods interpolate general function values defined on a manifold point cloud. In order to extent the manifold, and therefore generalize it to a large region, some methods follow a multi-scale approach. This multi-scale approach extents the manifold to a larger region in which the manifold is "simple" and restricts it in "complicated" (i.e. highly non-linear) regions |cite|. Two examples of out-of-sample methods are the "geometric harmonics interpolation" with multiscale extension |cite| and the the Laplacian Pyramids |cite|, |cite|.
...
The second data type adressed in datafold are data with temporal context sampled from a dynamical system. In this case a data-driven model aims to fit and generalize the underlying dynamics, which is also referred to "system identification". The formulation of a dynamica system includes a state, a rule of how to evolve a given state to future states and a phase space which is assumed to have a manifold structure. The temporal context and inherent order of time series data require a different handling compared to point clouds. However, in order to describe the phase space, models for point cloud data become relevant again and improve  the accuracy to identify a dynamical system from data.

datafold focuses on the Dynamic Mode Decomposition (DMD) |cite|. The DMD lineraly decomposes the available time series data into spatio-temporal components, which then define a linear dynamical system. Many DMD based variants address to in general non-linearity in the underlying system. This is usually done by changing the time series coordinates in a pre-step before the DMD is applied |Hankel|, |KernelDMD|, |Higher order DMD|, |...|. The justification of this workflow is covered by operator theory and functional analysis, specifically the Koopman operator. The connection of DMD to the Koopman operator is explicitly made clear in the generalizing numerical method Extended Dynamic Mode Decomposition (E-DMD) |cite|. In contrast to a non-linear flow operator of the dynamical system, the Koopman operator acts linearly on a functional space, the so-called obserable space. The Koopman view on a dynamical system is exact if the basis of the observable space is infinite dimensional. In practice, the E-DMD approximates the Koopman operator with a matrix, based on a finite set of functionals, the so-called dictionary. The functional representation of the dictionary is a change of coordinates, in which in an optimal case the dynamics linearize. In other words, the dictionary contains observable functions that linearlize the dynamical system's phase space. This allows the approximated Koopman matrix to describe a non-linear dynamical systems in that coordinate system. 

However, finding a good choice of dictionary is comparable to the machine learning task of "model selection". In addition to linearizing the phase space, often there further reasons that make processing the time series data necessary. In the case of heterogenous data, the time series, the feature may be required to be scaled. Another important reoccuring preprocessing is time delay embedding, which is required if the actual time samples are not samples of the phase space. In this case, the Takens theorem, a delay embedding can reconstruct a diffeomorphic manifold of the phase space. In summary, obtaining and linearizing the phase space manifold can require a flexible number transformations and therefore requires this flexibility of a software implementation.
