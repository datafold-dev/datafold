...
Increasing data availability has changed the analysis and interpretation of data in many scientific fields and practical applications.  While the analyzed complex systems, such as pyhsical systems, remain the same, the number of measurements increase both in quantity and dimension. For systems where the governing equations are unknown or untractable, a data-driven analysis can provide new insights if it allows to reveal the underlying intrinsic geometrical structure. A non-parametric model may as well only serve as a  surrogate model for complex application tasks, such as the prediction of a dynamical system learnt from data. In contrast to the equation-driven analysis, this can decrease the research time and cost of finding the principal equations for a system. However, the increasing data availability also creates challanges in the entire analysis workflow from processing, extracting, to interpreting the data. 

In mathematical terms the geometric structure encoded in the data is referred to as a "manifold". All data-driven models assume that there is some pattern or manifold structure in the data. Machine learning is then the process of building numerical models that adapt to this underyling structure and solve tasks like regression, classification or prediction. We can classify machine learning models in how they incorporate and presume the geometrical form. On the one hand black box models have an implcit presumption of structure in the data, but do not allow this insight to be revealed. A canonical example of black box model are feedforward neural networks, which can provide great predictive power as they generalize to highligh non-linear data, but the model itself does not reveal the intrinsic manifold. On the other hand, there are model types that explicitly incorporate manifold structure in the data. A parametrization of a data manifold which also generalizes to regions outside the available data samples, can then provide a path to uncover information about the intrinsic behaviour of the underlying system. A further advantage is that it becomes possible to incorporate prior knowledge of the data and system. For example a measure of proximity between samples in a kernel which describes the data locality and therefore the hidden manifold.
